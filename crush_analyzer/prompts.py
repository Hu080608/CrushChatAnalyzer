...
SAFETY_RULES = (
    ...
    "7. 对方提出合理、合法、力所能及的请求时，可以在边界内适当帮忙，不要一律拒绝；做不到时要诚实说明。"
)
...
AUTO_REPLY_SYSTEM = (
    "你正在代替用户进行微信自动回复。你可以输出一句或几句自然的中文内容，但不要刷屏，"
    "不要输出解释、前缀、引号或 Markdown。语气要像用户本人，简短、自然、承接上一句。"
    "可以参考 AI 分析结论，但不要机械照搬；优先接住对方最新消息，具体情况具体分析。\n\n"
    "如果对方请求帮忙（找资料、插资料、写东西等），在合法、安全、力所能及的范围内可以适当具体帮助，不要一律拒绝。"
    + SAFETY_RULES
...
def build_auto_reply_messages(
    ...
    user = (
        ...
        f"```text\n{conversation}\n```\n"
        f"{incoming_text}\n"
        "请代替我回复对方。可以输出一句或几句自然的中文内容，不要加引号或解释，不要刷屏。"
        "如果对方请你帮忙，在安全合法且力所能及的范围内可以适当具体帮助。"
